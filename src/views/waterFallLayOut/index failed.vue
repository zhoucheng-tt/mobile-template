...
export default {
  ...
  methods: {
    ...
    waterFall () {
      ...
      for (let i = 0; i < item.length; i++) {
        ...
        if (i < columnCount) {
          ...
          if (Number(item[i].style.left.split(unit)[0]) === Number(Math.floor((itemWidth * 0.05) / 2))) {
            leftArr.push({ id: i + 1, height: item[i].style.height })
          } else {
            rightArr.push({ id: i + 1, height: item[i].style.height })
          }
          // 设置第二行的宽度
          item[i].style.width = itemWidth * 0.95 + unit
          // 设置顶部距离
      ...
      const leftFirstArr = []
      leftFirstArr.push(this.contentList[0])
      // 左侧加入第一个
      const leftArrAfter = [...leftFirstArr, ...leftArr]
      let leftHeight = 0
      leftArrAfter.forEach((itemLeft, indexLeft) => {
        const left = Number(indexLeft * topGap)
        const leftTop = leftHeight + Number(left)
        item[Number(itemLeft.id) - 1].style.top = leftTop + unit
        leftHeight = leftHeight + Number(item[itemLeft.id - 1].style.height.split(unit)[0])
      })
      const rightFirstArr = []
      rightFirstArr.push(this.contentList[1])
      // 右侧加入第一个
      const rightArrAfter = [...rightFirstArr, ...rightArr]
      let rightHeight = 0
      rightArrAfter.forEach((itemRight, indexRight) => {
        const left = Number(indexRight * topGap)
        const rightTop = rightHeight + Number(left)
